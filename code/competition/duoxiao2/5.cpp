...
inline bool outside(,int x1,int x2,int y1,int y2){
    return xl>x2||xr<x1||yl>y2||yr<y1;
}
ll query(int x,int tx,int ty,int r){
    if(x==0)
        return 0;
    if(outside(tx,ty,r,mi[x][0],ma[x][0],mi[x][1],ma[x][1]))return 0;
    if(inside(tx,ty,r,mi[x][0],ma[x][0],mi[x][1],ma[x][1]))return sum[x];
    ll ans=query(ls[x],tx,ty,r)+query(rs[x],tx,ty,r);
    if(inside(tx,ty,r,tree[x].dim[0],tree[x].dim[0],tree[x].dim[1],tree[x].dim[1]))
        ans+=tree[x].val;
    return ans;
}
int main(){
    ...
    int v;
    for(int i=1;i<=n;i++){
        rd(x[i]),rd(y[i]),rd(v),rd(r[i]);
        insert(root,(node){x,y,z});
    }
    for(int i=1;i<=n;i++){
        wt(query(root,x[i],y[i],r[i])),pt('\n');
    }
}