...
int t;
int n,m;
void dfs(int step,int fr,int to){
    if(step==n){
        tmp.add_edge(fr,to,1,t);
        return;
    }
    int tt=-1;
    if(t2[step]=='-')
    ...
    if(t1[step]=='0')
        for(int i=0;i<=1;i++)
            dfs(step+1,(fr<<1)+i,(to<<1)+(tt==-1?i:tt));
    else if(t1[step]=='-'){
        dfs(step+1,(fr<<1)+0,(to<<1)+(tt==-1?0:tt));
    }else dfs(step+1,(fr<<1)+1,(to<<1)+(tt==-1?1:tt));
}
void work()
{
    cin>>n>>m;
    tmp.init((1<<n)+4,(1<<n)-1,1<<n);
    tmp.add_edge(0,(1<<n),1,0);
    for(int i=1;i<=m;i++){
        cin>>t>>t1>>t2;
        dfs(0,0,0);
    }
