...
{
    cin>>n;
    tmp.init(n*3+10,0,n*3+5);
    int tt;
    for(int i=1;i<=n;i++)
        cin>>date[i];
    cin>>p>>m>>f>>nn>>s;
    for(int i=1;i<=n;i++){
        tmp.add_edge(0,i,date[i],0);
        tmp.add_edge(i+n,n*3+5,date[i],0);
        if(i!=n)
            tmp.add_edge(i,i+1,inf,0);
        tmp.add_edge(0,i+n,inf,p);
        if(i+nn<=n)
            tmp.add_edge(i,(i+nn)+n,inf,s);
        if(i+m<=n)
            tmp.add_edge(i,(i+m)+n,inf,f);
    }
    tmp.cost_flow();
    cout<<tmp.ans<<endl;
}
